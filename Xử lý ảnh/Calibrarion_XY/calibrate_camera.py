...
calibrate_camera.py
--------------------
Calib camera bang bang co (checkerboard) 9x7 dung OpenCV.

Cach dung co ban (co the chay truc tiep khong can truyen tham so gi, vi tat ca
deu co gia tri mac dinh phu hop: thu muc anh = 'images', o vuong = 10mm):
    python calibrate_camera.py

Hoac tuy chinh:
    python calibrate_camera.py --images_dir images --board_w 9 --board_h 7 --square_size 10

Trong do:
    --images_dir    thu muc chua cac anh chup bang co (jpg/png), mac dinh 'images'
    --board_w       so GOC TRONG theo chieu ngang cua bang co, mac dinh 9
    --board_h       so GOC TRONG theo chieu doc cua bang co, mac dinh 7
    --square_size   kich thuoc canh 1 o vuong tren bang co, tinh bang mm, mac dinh 10mm
    --output        file luu ket qua calib (mac dinh: calibration_result.npz + .yaml)

Ve so goc trong:
    Neu ban noi "ban co 9x7" nghia la 9x7 O VUONG, thi so GOC TRONG (giao diem
    giua cac o den/trang) se la 8x6. OpenCV can so GOC TRONG, khong phai so o vuong.
    Neu 9x7 da la so giao diem thi giu nguyen --board_w 9 --board_h 7.

    Script se TU DONG thu ca 2 truong hop (9x7 va 8x6) tren tung anh va bao cho
    ban biet kich thuoc nao duoc phat hien nhieu nhat, de ban chon dung.

Ket qua tra ve:
    - Camera matrix (ma tran noi tai): fx, fy, cx, cy
    - Distortion coefficients (he so meo ong kinh): k1, k2, p1, p2, k3
    - Reprojection error (sai so trung binh) de danh gia chat luong calib
    - File .npz va .yaml de dung lai cho cac buoc xu ly anh/video sau nay
"""
